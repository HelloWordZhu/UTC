...
namespace CocaCola_UTC_Utility
{
    ...
    {
        static string strConn = ConfigurationManager.AppSettings["CocaCola_UTC"].ToString();
        ...
        /// 执行一个数组SQL语句的事务处理
        /// </summary>    
        public static int ExecuteTransactionInt(ArrayList list)
        {
            //string sConnStr = System.Configuration.ConfigurationManager.AppSettings[strConn];
            SqlConnection Conn = new SqlConnection(strConn);
            int num3;
            SqlCommand command = new SqlCommand();
            //this.NewConn(ref Conn);
            ...
            try
            {
                ...
                {
                    command.CommandText = list[num].ToString();
                    int tempi = command.ExecuteNonQuery();
                    if (tempi > 0)
                    {
                        num2 += tempi;
                    }
            ...
            catch (Exception exception)
            {
                ...
                throw new Exception(num + exception.Message.ToString());
            }
            finally
            {
                transaction.Dispose();
